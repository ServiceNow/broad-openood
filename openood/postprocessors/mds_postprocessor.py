...
class MDSPostprocessor(BasePostprocessor):
    ...
    def extract_stats(self, net, save_pth, ood_data_loader): 
        ...
        M_out = get_all_Mahalanobis_scores(
            net, ood_data_loader, self.num_classes,
            self.feature_mean, self.feature_prec,
            self.transform_matrix, self.num_layer, self.feature_type_list,
            self.magnitude)
        for layer_index in range(self.num_layer):
            
            file_pth = os.path.join(save_pth, 'layer_' + str(layer_index) + '.npy')
            np.save(file_pth, M_out[layer_index])
...
def get_all_Mahalanobis_scores(model, test_loader, num_classes, sample_mean,
                               precision, transform_matrix, n_layers,
                               feature_type_list, magnitude):
    '''
    Compute the proposed Mahalanobis confidence score on input dataset
    return: Mahalanobis score from layer_index
    '''
    model.eval()
    Mahalanobis = [[] for _ in range(n_layers)]
    for batch in tqdm(test_loader,
                      desc=f'{test_loader.dataset.name}'):
        data = batch['data'].cuda()
        data = Variable(data, requires_grad=True)
        all_noise_gaussian_score = compute_all_Mahalanobis_score(
            model, data, num_classes, sample_mean, precision, transform_matrix,
            feature_type_list, magnitude)
        for layer_index, layer_scores in enumerate(Mahalanobis):
            layer_scores.extend(all_noise_gaussian_score[layer_index].cpu().numpy())
    return Mahalanobis


def compute_all_Mahalanobis_score(model,
                                  data,
                                  num_classes,
                                  sample_mean,
                                  precision,
    ...
    for layer_index, out_feat in enumerate(out_features):
        out_feat = process_feature_type(out_feat,
                                        feature_type_list[layer_index])
        out_feat = torch.mm(out_feat, transform_matrix[layer_index])

        # compute Mahalanobis score
        gaussian_score = 0
        for i in range(num_classes):
            batch_sample_mean = sample_mean[layer_index][i]
            zero_f = out_feat.data - batch_sample_mean
            term_gau = -0.5 * torch.mm(torch.mm(zero_f, precision[layer_index]),
                                    zero_f.t()).diag()
            if i == 0:
                gaussian_score = term_gau.view(-1, 1)
        ...
        sample_pred = gaussian_score.max(1)[1]
        batch_sample_mean = sample_mean[layer_index].index_select(0, sample_pred)
        zero_f = out_feat - Variable(batch_sample_mean)
        pure_gau = -0.5 * torch.mm(
            torch.mm(zero_f, Variable(precision[layer_index])), zero_f.t()).diag()
        loss = torch.mean(-pure_gau)
        if layer_index == len(out_features)-1:
            loss.backward()
        else:
        ...
        noise_gaussian_score, _ = torch.max(noise_gaussian_score, dim=1)
        all_layers_score.append(noise_gaussian_score)
