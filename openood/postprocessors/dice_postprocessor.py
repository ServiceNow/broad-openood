...
class DICEPostprocessor(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data: Any):
        if self.config.network.name == 'resnet50':
            ...
            fc = net.head.layers.head
        if self.masked_w is None:
            self.calculate_mask(fc.weight)
        _, feature = net(data, return_feature=True)
        vote = feature[:, None, :] * self.masked_w
        output = vote.sum(2) + fc.bias
    ...
    def extract_stats(self, net, save_pth, ood_data_loader):
        ...
        for batch in ood_data_loader:
            ...
            dice_scores.extend(score.cpu().tolist())
        dice_scores = np.array(dice_scores)

        dice_file_pth = os.path.join(save_pth, 'dice.npy')
        np.save(dice_file_pth, dice_scores)
